...
y.obs.random <- function(rou, tau,
                         controlNumber, timeLength, y.obs.v) {
...
# #curren test
# rou = 0; tau = 0.6; 
# y.obs.ran <- y.obs.random(rou = rou, timeLength = timeLength , tau = tau, controlNumber = controlNumber , y.obs.v = y.obs.v)
...
###
SCM.sample.random <- function(data.raw, rou, timeLength ){ 
...
final.plotdata <- function(
            ...
            final <- data.frame(data, "Treat.Hat_SCM.rand" = SCM.sample.random(data.raw = data , rou = rou,  timeLength = timeLength)$result$Y_scm.hat )
